...
/**
 * Inline code spans carry pairs shaped exactly like a qualified reference but
 * which are not one — `owner:repo`, `host:port`, `key:value`. Left in, every
 * one of them is reported as a missing skill, and a check that cries wolf is a
 * check people learn to skip. Removed (blanked to a space so word boundaries
 * survive) before the QUALIFIED pass. The run-length backreference matches
 * ``code with a ` inside`` spans too. The backticked-skill form is defined by
 * its backticks, so it is matched on the un-stripped text instead.
 */
...
export function extractReferences(name: string, body: string): string[] {
  ...
  // QUALIFIED matches any `word:word`, so inline code — where owner:repo and
  // host:port live — is stripped first. Bare prose pairs (a `host:port` written
  // without backticks) still match by design: NOT_NAMESPACES catches the common
  // language tags, and asking authors to backtick a literal pair is cheaper than
  // a heuristic that guesses which prose colons are references.
  for (const match of withoutInlineCode(prose).matchAll(QUALIFIED)) {
