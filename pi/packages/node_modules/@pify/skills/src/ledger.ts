...
export function commitLedger(file: string, pending: Ledger): Ledger {
  ...
  try {
    ...
    // The rename can fail on Windows (EPERM/EBUSY) when another session or an
    // AV scanner momentarily holds the target. The pid-named temp would then
    // linger forever, so drop it before letting the caller retry on next save.
    try {
      unlinkSync(tmp);
    } catch {
      // Best effort: if even the cleanup fails there is nothing more to do.
    }
    throw err;
  }
  return merged;
