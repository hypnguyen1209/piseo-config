 ...
 *
 * `measured` says where the skill set came from. Once pi has built a request it
 * hands the extension the exact set it loaded (packages, `.agents/skills`,
 * enable/disable and trust already applied); before that the numbers are a disk
 * scan that sees less, so the difference is labelled rather than hidden.
 */
export function formatOverview(
  ...
  if (inventory.costs.length === 0) {
    return "No skills loaded. pi reads them from .pi/skills, .agents/skills, <agentDir>/skills, and installed packages.";
  }
  ...
  lines.push(
    "",
    measured
      ? "Measured from what pi loaded on the last request."
      : "From disk (before the first request) — this may differ from what pi loads once it builds one.",
  );
