 ...
 * which the spec requires the `name` field to match. `fileName` is the skill's
 * own file name: pi will load a bare `<root>/<name>.md` as a skill, but the
 * spec is `<name>/SKILL.md` — such a file has no directory to match against, so
 * it gets its own message rather than a phantom name-mismatch error.
 */
export function checkSkill(text: string, directory: string, fileName = "SKILL.md"): Problem[] {
  ...
  // A bare .md skill loads in pi but is outside the Agent Skills spec, which
  // requires a `<name>/SKILL.md` layout. Name it plainly; do not silence the
  // portability problem by pretending the file conforms.
  if (fileName !== "SKILL.md") {
    problems.push({
      level: "error",
      message: `"${fileName}" is a bare .md skill; the spec expects <name>/SKILL.md (pi loads it, other hosts will not)`,
    });
  }
  ...
  if (!name) {
    ...
    // Only a real SKILL.md has a directory the name is meant to match; a bare
    // .md was already reported above and has no such directory.
    if (fileName === "SKILL.md" && directory && name !== directory) {
      problems.push({
        level: "error",
