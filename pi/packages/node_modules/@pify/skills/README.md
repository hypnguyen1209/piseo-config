...
The check is deliberately conservative, because a check that cries wolf is one people learn to skip. It reads the two forms collections actually write — `namespace:skill-name` and `` `skill-name` skill`` — ignores bare paths like `skills/foo` that are as often documentation links, and skips fenced code blocks: a fence opened with ```` ```json:metadata ```` looks exactly like a reference, and did read as one until a real collection proved it. Inline code spans are skipped the same way, so a `` `owner:repo` `` or `` `host:port` `` you show in backticks is not mistaken for a missing skill. A colon pair written as bare prose still counts — backtick your literals and it stays quiet.
...
Several pi windows on the same project share the one file: each folds in its own firings without overwriting the others', and `/skills` re-reads the file before answering, so it shows every session's firings and not only the ones from the window you asked in.

## Where the numbers come from

pi loads skills from more places than one directory: project `.pi/skills` and `.agents/skills` (up the ancestor chain), user `~/.pi/agent/skills` and `~/.agents/skills`, and every installed package's skills — filtered by per-resource enable/disable, project trust, and `--skill`/`--no-skills`. Re-discovering that set correctly is pi's job, not a thing to guess at, so this package does not try.

Instead it takes the exact set pi built into the system prompt: pi hands it to extensions on every request, packages and all, with trust and enable/disable already applied. So `/skills` reports **what pi loaded on the last request** — the honest answer even when it is not the expected one. If a project's skills seem to be missing, that is usually project trust: pi asks before reading `.pi/skills`.

Before the very first request there is nothing to take yet, so `/skills` falls back to a disk scan of `.pi/skills` and `<agentDir>/skills` and labels the number "from disk" rather than presenting a subset as the whole.
